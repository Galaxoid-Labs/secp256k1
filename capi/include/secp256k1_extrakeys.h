/*
 * X-only public keys and keypairs, as used by BIP340 and Taproot.
 * Mirrors upstream's secp256k1_extrakeys.h.
 */
...
/** An opaque x-only public key: a point identified by its x coordinate, with y even. */
typedef struct secp256k1_xonly_pubkey {
    unsigned char data[64];
} secp256k1_xonly_pubkey;

/** An opaque keypair, holding both halves so signing need not re-derive the public key. */
